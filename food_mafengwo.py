...
def get_info(html,id):
    selector=etree.HTML(html)
    data={}
    #记录餐厅链接
    url = 'https://www.mafengwo.cn/poi/' + id
    data['Link']=url
    #获取餐厅名字
    ...
    try:
        ...
        for page in range(1,8):
            try:
                source='https://www.mafengwo.cn/gonglve/ajax.php?act=get_poi_comments&poi_id='+id+'&type=0&order=2&category=&page='+str(page)
                ...
                texts=text.split('<\/p>')
                for text in texts:
                    if text.find(r'<p class=\"rev-txt\">'):
                        line=text.split(r'<p class=\"rev-txt\">')[-1]
                        if(line[0:2]==r"\u"):
                            line=line.encode('utf-8','ignore').decode('unicode-escape').strip()
                            line=line.replace('\n','')
...
for num in range(start,start+n):
    ...
    for each in list:
        ...
        id=each.get_attribute('href')[26:].split('?')[0]
        html=get_html(link)
        try:
            #获取分类和所处商圈
            category=each.text.split('\n')[2].split()
            business_district=each.text.split('\n')[3]
            data=get_info(html, id)
